...
public class TreeLayout {
    ...
    public static final int NODE_VERTICAL_SPACING = 28;
    ...
    public static TreeVolume layout(MaterialNode root, long batches, long parentAmount, int depth, ChanceState chance) {
        ...
        if (amount <= 0) amount = root.amount;

        if (root.children != null && !root.children.isEmpty()
            && root.state == FoldState.EXPANDED
            && root.recipe != null) {
            ...
            if (combined != null) {
                ...
                int headX = (combined.getMinLeft() + combined.getMaxRight()) / 2 - NODE_WIDTH / 2;
                int headY = depth * NODE_VERTICAL_SPACING;
                TreeNode head = new TreeNode(root, headX, headY, NODE_WIDTH, NODE_WIDTH / 2, amount, chance);
        ...
        TreeNode node = new TreeNode(root, x, y, NODE_WIDTH, NODE_WIDTH / 2, amount, chance);
        volume.nodes.add(node);
        volume.updateBounds(depth, x, x + NODE_WIDTH);
        return volume;
    }
